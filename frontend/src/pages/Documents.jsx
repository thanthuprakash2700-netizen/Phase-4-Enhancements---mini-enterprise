...
const Documents = () => {
  ...
  const fetchDocuments = async (taskId, currentSkip = 0) => {
    try {
      const url = taskId ? `/documents/task/${taskId}` : `/documents/general`;
      const res = await api.get(`${url}?skip=${currentSkip}&limit=${limit}`);
  ...
  const handlePageChange = (newSkip) => {
    setSkip(newSkip);
    fetchDocuments(selectedTask, newSkip);
  };
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const url = selectedTask ? `/documents/upload?task_id=${selectedTask}` : `/documents/upload`;

...
const Kanban = () => {
  ...
  useEffect(() => {
    if (lastMessage && (lastMessage.type === 'TASK_CREATED' || lastMessage.type === 'TASK_UPDATED')) {
      fetchBoard();
    }
...
const TaskCard = ({ task, onEdit, onDelete, canManage }) => {
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-4">
        <div className="flex gap-2 flex-wrap">
          ...
          {task.delay_risk && task.delay_risk !== 'low' && (
            <span className={`text-[10px] uppercase font-bold px-2 py-1 rounded-lg border ${task.delay_risk === 'high' ? 'bg-orange-50 text-orange-600 border-orange-100' : 'bg-yellow-50 text-yellow-600 border-yellow-100'}`}>
      ...
      <h4 className="font-bold text-slate-800 mb-2 leading-tight group-hover:text-primary transition-colors flex items-center gap-2">
        {task.attention_required && <AlertCircle size={16} className="text-rose-500 animate-pulse shrink-0" />}
        {task.title}
      </h4>
